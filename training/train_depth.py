...
EXPECTED_CHECKPOINT_MAX_MB = 60.0
...
def main() -> None:
    ...
    try:
        ...
        model = DynamicFilterNetwork(in_channels=3).cuda(local_rank)
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            for step, (images, depth_maps) in enumerate(dataloader, start=1):
...
if __name__ == "__main__":
    if "WORLD_SIZE" not in os.environ:
        print("Please run with torchrun, e.g. torchrun --nproc_per_node=4 training/train_depth.py")
    else:
        main()
